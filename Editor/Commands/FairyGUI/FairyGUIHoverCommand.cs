...
{
    public sealed class FairyGUIHoverCommand : IGroupedCommand, IAsyncRpcCommand
    {
        ...
        public FairyGUIHoverCommand(IPanelSource source, IStageInput stageInput, IFrameStepper stepper)
        {
            this.source = source;
        ...
        public string Method => RpcMethods.FairyGuiHover;
        public string Group => "fgui.input";
        public string Action => "hover";

        public RpcToolDescriptor Descriptor => new RpcToolDescriptor
        {
            Name = "fgui-hover",
            RpcMethod = RpcMethods.FairyGuiHover,
            Title = "FairyGUI / Hover",
            Description = "Move the pointer over a FairyGUI object (no button) via the input pipeline. " +
                "Fires onRollOver/onRollOut/onTouchMove. Requires target visible/on-screen. Play mode only.",
            Completion = "response",
            FailureMode = "error",
